...
class GeneratedMaze:
    """Immutable raw maze data returned to the Pacman application."""
    ...
    @property
    def width(self) -> int:
        """Return the number of logical maze cells in one row."""
        return len(self.grid[0])

    @property
    def height(self) -> int:
        """Return the number of logical maze rows."""
...
def _freeze_grid(raw_grid: object, width: int, height: int) -> MazeGrid:
    """Validate and freeze the package's mutable bitmask grid."""
...
class MazeGeneratorAdapter:
    ...
    def generate(
        ...
        """Generate an imperfect maze and return validated raw data."""
        ...
        return GeneratedMaze(
            grid=_freeze_grid(generated.maze, width, height),
            entry=_validate_coordinate(
                generated.maze_entry,
                "entry",
                width,
                height,
            ),
            exit=_validate_coordinate(
                generated.maze_exit,
                "exit",
                width,
                height,
